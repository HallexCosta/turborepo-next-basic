...
export const TextInputAchievement = memo(({ id, content, workExperienceId, workExperienceIndex, index}: TextInputAchievementProps) => {
    ...
    const handleRemoveInput = () => {
        ...
        if (Number.isNaN(Number(id)))
            fetch(`http://localhost:3001/api/achievements/${workExperienceId}/${id}`, options)
    ...
    return (
        <div className="flex gap-1 items-center">
            <TextInput
                className="w-full"
                onChange={(e) => updateAchievement(workExperienceIndex, index, {
                ...
                // rows={10}
            />
            <button onClick={handleRemoveInput}
