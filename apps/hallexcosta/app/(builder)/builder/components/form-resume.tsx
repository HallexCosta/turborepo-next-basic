...
import {CVTemplatePage, CVTemplatePageEditable} from "../../../(curriculum)/cv/page";
    ...
    const onSaveResume = async (e) => {
        ...
        const handleCreateWorkExperiences = (workExperiences) => {
            ...
            return workExperiences
                .map(({achievements, ...workExperience}) => {
                    return {
                        ...workExperience,
        ...
        const handleCreateAchievements = (workExperiences) => {
            ...
            for (const {achievements, ...workExperience} of workExperiences) {
                data.push({
                    ...
                    achievementsWithContent: achievements.filter(achievement => achievement.content.length),
                    achievementsWithoutContent: achievements.filter(achievement => !achievement.content.length && Number.isNaN(Number(achievement.id)))
                })
            }
        ...
        const requestPersonUpdate = fetch(`http://localhost:3001/api/person/${username}`, {
            method,
            body: JSON.stringify({
                ...person,
        ...
        const requestWorkExperiences = fetch(`http://localhost:3001/api/work-experiences/${person.id}`, {
            method,
        ...
        const createFetchAchievementsRequest = (workExperienceId: number, achievements) => fetch(`http://localhost:3001/api/achievements/${workExperienceId}`, {
            method,
        ...
        //     headers
        // })
        const deleteFetchAchievementsRequest = (workExperienceId, achievementId) => fetch(`http://localhost:3001/api/achievements/${workExperienceId}/${achievementId}`, {
            method: 'DELETE',
            headers
        })

        for (const { workExperienceId, achievementsWithContent, achievementsWithoutContent } of handleCreateAchievements(workExperiences)) {
            console.log({ workExperienceId, achievementsWithContent, achievementsWithoutContent})
            requests.push(createFetchAchievementsRequest(workExperienceId, achievementsWithContent))
            for (const achievement of achievementsWithoutContent) {
                requests.push(deleteFetchAchievementsRequest(workExperienceId, achievement.id))
            }
        }

        const [,, workExperiencesResponse, ...achievementsResponses] = await Promise.all(requests)
        ...
        <form onSubmit={onSaveResume}>
            ...
            <fieldset>
                <legend className="mb-4 w-full flex items-center gap-4">
                    <span className="text-xl text-white font-bold">Experiência profissional</span>
            ...
            <div className="flex gap-4">
                <Button type="submit" className="w-full" color="blue">Salvar</Button>
                <Button className="w-full" color="light" onClick={() => {
                    // enablePreviewMode()
                    setOpenModal(true)
