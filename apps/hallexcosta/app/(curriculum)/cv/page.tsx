...
export function CVTemplatePageEditable({person, className = '' }) {
    return (
        <section className={twMerge("p-4 lg:px-0 lg:mx-auto lg:max-w-3xl bg-white", className)}>
            <div className="flex flex-col">
                <div className="who-is">
                    <h1 className="text-xl lg:text-5xl font-extrabold m-0 uppercase">{person?.name ?? person.person.name}</h1>
                    <h1 className="text-lg lg:text-4xl font-extrabold m-0 uppercase">{person?.role ?? person.person.role}</h1>
                </div>

                <div className="mt-2 flex items-center flex-wrap gap-1">
                    <Contact name="location">
                        {/*<Icons.Location/>*/}
                        <span className="text-xs">{person.contact?.city}, {person.contact.state}</span>
                        <Dot />
                    </Contact>
            ...
            <div>
                ...
                {person.workExperiences.map(workExperience => {
                    if (!_.isDate(workExperience.startDate)) workExperience.startDate = new Date(workExperience.startDate)
                    if (_.isString(workExperience.endDate)) workExperience.endDate = new Date(workExperience.endDate)

                    const startDate = dayjs(workExperience.startDate.toISOString()).format('MMM/YYYY')
                    let endDate: string | null = ''
                    if (workExperience.endDate) endDate = dayjs(workExperience.endDate.toISOString()).format('MMM/YYYY')
                    else endDate = null
            ...
            <div className="mt-6">
                ...
                <Experience.Root className="flex flex-col gap-4">
                    <Experience.Header
                        ...
                        endDate={dayjs(new Date('2022-12-10').toISOString()).format('MMM/YYYY')}
                    ...
                    <Experience.Header
                        ...
                        endDate={dayjs(new Date('2019-12-10').toISOString()).format('MMM/YYYY')}
                        currentActivity={false}
                    />
                    ...
                    <Experience.Header
                        ...
                        endDate={dayjs(new Date('2019-06-10').toISOString()).format('MMM/YYYY')}
                        currentActivity={false}
                    />
                    ...
                    <Experience.Header
                        ...
                        endDate={dayjs(new Date('2018-12-10').toISOString()).format('MMM/YYYY')}
                        currentActivity={false}
                    />
        ...
        <section className="p-4 lg:px-0 lg:mx-auto lg:max-w-4xl">
            ...
            <Experience.Root>
                ...
                <Experience.Content>
                    <Experience.Line>
                        {/*Sou uma pessoa entusiasmada e comprometida em aproveitar o poder da tecnologia para desenvolver soluções e resolver problemas. Minha experiência como Desenvolvedor PHP back-end inclui a implementação de novos fluxos na regra de negócios de plataformas, aprimoramento do layout front-end, automação de processos, integração de gateways de pagamento, provisionamento de infraestrutura utilizando a nuvem (Cloud) e implantação de serviços.*/}
                        Desenvolvedor full-stack com foco no back-end e um ano de experiência em aplicações PHP.
                        Familiarizado em tecnologias Javascript, como Node.js, Typescript, Express.js, Nest.js e
                        Next.js, além de habilidades em ferramentas de testes unitários. Possuo habilidades na
                        implantação de serviços com nginx como load balancer, uso eficiente do Docker e construção de
                        pipelines de implantação. Sou entusiasta e comprometido em aplicar a tecnologia para desenvolver
                        soluções e resolver problemas. Minha trajetória inclui implementação de novos fluxos na regra de
                        negócios, aprimoramento do layout front-end, automação de processos, integração de gateways de
            ...
            <Experience.Root>
                ...
                <Experience.Container>
                    ...
                    <Experience.Lines>
                        <Experience.Line>
                            Atuei na implementando de um novo fluxo na regra de negócios da plataforma. Nesse novo
                            contexto,
                        ...
                        <Experience.Line withDot>Implementação de um modulo de tradução permitindo o usuário escolher o
                            idioma inglês ou
                            português para utilizar o aplicativo.</Experience.Line>
                        <Experience.Line withDot>Implementação de funcionalidade de recuperação de senha através do
                            E-mail.</Experience.Line>
                        <Experience.Line withDot>Implementação filas/background jobs utilizando o Redis com
                            Yii2.</Experience.Line>
                        <Experience.Line withDot>Automatização no fluxo de Done, para auto confirmar caso passe 2 dias
                            que o evento foi
                        ...
                        <Experience.Line withDot>Implementando uma nova logica no filtro de pesquisa para buscar músicos
                            que possua pelo menos
                            um dos estilos musicais aplicados no filtro.</Experience.Line>
                        <Experience.Line withDot>Implementação da Push Notifications, para notificar os dispositivos de
                            um usuário sobre alguma
                            atividade prolongada ou que aconteça em background como confirmação de pagamento,
                            recebimentos,
                            mensagens, solicitações de eventos (aceitos, recusados, cancelados e
                            concluídos).</Experience.Line>
                        <Experience.Line withDot>Provisionamento e escalonamento de três novas instâncias do back-end e
                            oito do front-end.</Experience.Line>
                        <Experience.Line withDot>Implementação de politica de cancelamento e estornos em eventos, que
                            são aplicados tanto ao
